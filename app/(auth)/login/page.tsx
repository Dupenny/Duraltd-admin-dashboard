...
export default function LoginPage() {
  ...
  async function submitOtp(e: React.FormEvent) {
    ...
    if (Object.keys(errs).length) { setErrors(errs); return; }
    if (otp !== MOCK_OTP) { setErrors({ code: "Incorrect verification code. Please try again." }); return; }
    setLoading(true);
  ...
  function pickProduct(id: string) {
    setProduct(id as "durapayment" | "durapay" | "durabiz");
  ...
  return (
    <div style={{
      ...
      <div style={{ position:"fixed", top:"40%", left:"30%", width:300, height:300, borderRadius:"50%", background:"radial-gradient(circle, rgba(16,185,129,0.04), transparent 70%)", pointerEvents:"none" }} />
        ...
        <div style={{ background:"#fff", borderRadius:22, border:"1px solid #E2E8F4", boxShadow:"0 8px 40px rgba(13,27,62,0.09)", overflow:"hidden", animation:"fadeUp 0.4s ease" }}>
          ...
          {step === "credentials" && (
            <div className="login-card-inner">
              {/* Top accent bar */}
              <div style={{ height:4, background:"linear-gradient(90deg,#2563EB,#4F46E5,#7C3AED)", borderRadius:"4px 4px 0 0", margin:"-40px -40px 36px" }} className="accent-bar" />
              ...
              <form onSubmit={submitLogin} style={{ display:"flex", flexDirection:"column", gap:20 }}>
                ...
                <button type="submit" className="btn btn-primary" disabled={loading} style={{ width:"100%", justifyContent:"center", padding:"14px", fontSize:15, marginTop:4, borderRadius:12, background:"linear-gradient(135deg,#2563EB,#4F46E5)", border:"none", color:"#fff", fontWeight:700, cursor: loading ? "not-allowed" : "pointer", display:"flex", alignItems:"center", gap:8, transition:"all 0.2s", boxShadow:"0 4px 16px rgba(37,99,235,0.3)" }}>
              ...
              <div style={{ marginTop:28, paddingTop:24, borderTop:"1px solid #F4F7FE", textAlign:"center" }}>
                ...
                <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:16, marginTop:10 }}>
                  <span style={{ display:"flex", alignItems:"center", gap:5, fontSize:12, color:"#8A97B0" }}><ShieldIcon />256-bit SSL</span>
                  <span style={{ width:3, height:3, borderRadius:"50%", background:"#E2E8F4" }} />
                  <span style={{ display:"flex", alignItems:"center", gap:5, fontSize:12, color:"#8A97B0" }}><LockSmIcon />End-to-end encrypted</span>
                </div>
              </div>
            </div>
          )}
          ...
          {step === "otp" && (
            <div className="login-card-inner">
              ...
              <form onSubmit={submitOtp} style={{ display:"flex", flexDirection:"column", gap:18 }}>
                ...
                <button type="submit" className="btn btn-primary" disabled={loading || otp.length < 6} style={{ width:"100%", justifyContent:"center", padding:"14px", fontSize:15, borderRadius:12, background:"linear-gradient(135deg,#2563EB,#4F46E5)", border:"none", color:"#fff", fontWeight:700, cursor: (loading || otp.length < 6) ? "not-allowed" : "pointer", display:"flex", alignItems:"center", gap:8, opacity: otp.length < 6 ? 0.6 : 1, transition:"all 0.2s", boxShadow:"0 4px 16px rgba(37,99,235,0.3)" }}>
                  {loading ? <><SpinIcon />Verifying…</> : "Verify & Continue →"}
                </button>
              </form>
              <p style={{ textAlign:"center", marginTop:22, fontSize:13, color:"#8A97B0" }}>
                Didn't receive it?{" "}
