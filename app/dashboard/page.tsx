        ...
        <>
          ...
          <div
            style={{
              ...
              borderRadius: 14,
              boxShadow: "0 12px 40px rgba(13,27,62,0.12)",
              minWidth: 200,
              overflow: "hidden",
...
function ChartTip({ active, payload, label }: any) {
  ...
  return (
    <div
      ...
      <p style={{ fontWeight: 600, color: "var(--text)", marginBottom: 5 }}>
        {label}
      </p>
      {payload.map((p: any) => (
  ...
  function exportCSV() {
    ...
    const rows = filtered.map((tx) =>
      cols
        .map((c) => {
          if (c.id === "amount")
            return perms.canViewRevenue ? fmtCurrency(tx.amount) : "—";
    ...
    a.href = URL.createObjectURL(
      new Blob([[cols.map((c) => c.label).join(","), ...rows].join("\n")], {
        type: "text/csv",
      }),
    );
  ...
  const KPIS = [
    {
      ...
      value: fmtCurrency(kpiData.avgOrderValue ?? 72980, true),
      delta: "+3.16%",
      pos: true,
      icon: "💰",
      dark: true,
    ...
    {
      label: "Total Orders",
      value: fmtNum(kpiData.totalOrders ?? 2219),
      delta: "+1.18%",
      pos: true,
      icon: "🛍️",
      dark: false,
    },
    {
      ...
      value: fmtNum(kpiData.activeCustomers ?? 1847),
      delta: "+6.4%",
      pos: true,
      icon: "👥",
      dark: false,
    },
    {
      ...
      value: perms.canViewRevenue
        ? fmtCurrency(kpiData.platformRevenue ?? 560000, true)
        : "—",
      delta: "+2.42%",
      pos: true,
      icon: "📈",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div className="animate-fade-up">
          <div
            ...
            <ProductBanner
              product={product}
              loading={dash.loading || txHook.loading}
            />
          </div>
          <p
            style={{
        ...
        {/* API error banners */}
        {dash.error && (
          <ErrorBanner message={dash.error} onRetry={dash.refetch} />
        )}
        {txHook.error && (
          <ErrorBanner message={txHook.error} onRetry={txHook.refetch} />
        )}
          ...
          {KPIS.map(({ label, value, delta, pos, icon, dark }) => (
            <div
              ...
              <div
                ...
                <span
                  style={{ fontSize: "clamp(16px,4vw,20px)", lineHeight: 1 }}>
                  {icon}
                </span>
              </div>
              <div
                ...
                {(["area", "bar"] as const).map((t) => (
                  <button
                    ...
                    style={{
                      ...
                      background:
                        chartType === t ? "var(--surface)" : "transparent",
                      color: chartType === t ? "var(--text)" : "var(--text-3)",
                      boxShadow:
                        chartType === t ? "0 1px 4px rgba(0,0,0,0.08)" : "none",
        ...
        <div
          ...
          <div className="orders-bar">
            ...
            <div className="orders-actions">
              ...
              <Dropdown
                ...
                trigger={
                  <button
                    ...
                    style={{
                      fontSize: 12,
                      background: customizeOpen ? "var(--accent)" : undefined,
                      color: customizeOpen ? "#fff" : undefined,
                    }}>
                    ...
                    <button
                      ...
                      <div
                        style={{
                          ...
                          borderRadius: 4,
                          flexShrink: 0,
                          border: `2px solid ${visibleCols.includes(col.id) ? "var(--accent)" : "var(--border-strong)"}`,
                          background: visibleCols.includes(col.id)
                            ? "var(--accent)"
                            : "var(--surface)",
                          display: "flex",
                          alignItems: "center",
                    ...
                    {STATUS_OPTS.map((s) => (
                      <button
                        ...
                        style={{
                          fontSize: 11,
                          background:
                            statusFilter === s ? "var(--accent)" : undefined,
                    ...
                    {PRODUCT_OPTS.map((p) => (
                      <button
                        ...
                        style={{
                          fontSize: 11,
                          background:
                            productFilter === p ? "var(--accent)" : undefined,
                          color: productFilter === p ? "#fff" : undefined,
                        }}>
                        {p}
                      </button>
                    ))}
              ...
              <Dropdown
                ...
                <div>
                  {[
                    { l: "Export as CSV", fn: exportCSV, c: "#10B981" },
                    { l: "Export as JSON", fn: exportJSON, c: "#2563EB" },
                  ].map((e, i) => (
                    <button
                      key={e.l}
                      onClick={e.fn}
                      ...
                      <span style={{ color: e.c, fontWeight: 700 }}>↓</span>{" "}
                      {e.l}
                    </button>
                  ))}
                </div>
          ...
          <div className="orders-table-wrap">
            <table className="data-tbl" style={{ minWidth: 480 }}>
              <thead>
                <tr>
                  {ALL_COLS.filter((c) => visibleCols.includes(c.id)).map(
                    (col) => (
            ...
            <div style={{ display: "flex", gap: 4 }}>
              ...
              {Array.from({ length: pages }, (_, i) => i + 1).map((p) => (
                <button
                  ...
                  style={{
                    fontSize: 12,
                    background: page === p ? "var(--text)" : undefined,
        ...
        {perms.canViewBalance && (
          <div
            ...
            <p
              ...
              {balData.total
                ? new Intl.NumberFormat("en-NG").format(balData.total)
                : "847,200,000"}
            </p>
            <p
              style={{
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {[
                { l: "DuraPayment", v: "₦312M", c: "#60A5FA" },
                { l: "DuraPay", v: "₦284M", c: "#34D399" },
                { l: "DuraBiz", v: "₦251M", c: "#A78BFA" },
              ].map((b) => (
                <div
                  key={b.l}
                  style={{
