...
interface NavItem {
  ...
  requirePerm?: keyof ReturnType<typeof getPerms>; badge?: string;
}
...
const MAIN_NAV: NavItem[] = [
  ...
  { href: "/dashboard/issues",       label: "Shipment",         icon: <TruckIcon />, badge: "6" },
];

const SETTINGS_NAV: NavItem[] = [
  { href: "/dashboard/settings",  label: "Store Setting",    icon: <GearIcon />,   requirePerm: "canManageSettings" },
  { href: "/dashboard/analytics", label: "Platform Partner", icon: <GridSmIcon />, requirePerm: "canViewAnalytics" },
  { href: "/dashboard/users",     label: "Feedback",         icon: <ChatIcon />,   requirePerm: "canManageUsers" },
];

// Defined OUTSIDE parent — fixes the double-active bug
function NavLink({ item, pathname, mobile, onClose }: {
  item: NavItem; pathname: string; mobile?: boolean; onClose?: () => void;
}) {
  ...
  return (
    <Link href={item.href} onClick={mobile ? onClose : undefined}
      ...
      {item.badge && !active && (
        <span style={{ background: "var(--red)", color: "#fff", borderRadius: 20, padding: "1px 7px", fontSize: 10, fontWeight: 700 }}>
          {item.badge}
        </span>
      )}
    </Link>
    ...
    <aside style={{
      ...
      <div style={{ padding: "18px 16px 14px", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, flexShrink: 0, background: "linear-gradient(135deg,#1e40af,#3b82f6)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 10px rgba(37,99,235,0.35)" }}>
            <span style={{ color: "#fff", fontWeight: 800, fontSize: 16, fontFamily: "'Sora',sans-serif" }}>D</span>
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: 15, color: "var(--text)", letterSpacing: -0.3, lineHeight: 1.2 }}>
              Dura<span style={{ color: "var(--accent)" }}>LTD</span>
            </div>
            <div style={{ fontSize: 10, color: "var(--text-3)", fontWeight: 500 }}>Admin Console</div>
          </div>
          {mobile && (
            <button onClick={onClose} style={{ width: 30, height: 30, borderRadius: 8, background: "var(--surface-2)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "var(--text-3)", flexShrink: 0 }}>
              <XIcon />
            </button>
          )}
        </div>
      ...
      {user && (
        <div style={{ padding: "12px 12px 6px", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 12, background: "var(--surface-2)", border: "1px solid var(--border)" }}>
            <div style={{ width: 34, height: 34, borderRadius: 9, background: user.avatarColor, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 12, fontWeight: 700, flexShrink: 0, boxShadow: `0 2px 8px ${user.avatarColor}55` }}>
              {user.initials}
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 10.5, color: ROLE_LABELS[user.role].color, fontWeight: 600, marginTop: 1 }}>{ROLE_LABELS[user.role].label}</div>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Nav */}
      <nav style={{ flex: 1, padding: "10px 10px 8px", overflowY: "auto", overflowX: "hidden" }}>
        <p style={{ fontSize: 9.5, fontWeight: 700, color: "var(--text-3)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 5, padding: "0 4px" }}>Main Menu</p>
        <div style={{ display: "flex", flexDirection: "column", gap: 1, marginBottom: 18 }}>
          {visibleMain.map(item => <NavLink key={item.href} item={item} pathname={pathname} mobile={mobile} onClose={onClose} />)}
        </div>

        {visibleSettings.length > 0 && (
          <>
            <p style={{ fontSize: 9.5, fontWeight: 700, color: "var(--text-3)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 5, padding: "0 4px" }}>Settings</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
              {visibleSettings.map(item => <NavLink key={item.href} item={item} pathname={pathname} mobile={mobile} onClose={onClose} />)}
            </div>
          </>
        )}
      </nav>
      ...
      <div style={{ padding: "0 10px 10px", flexShrink: 0 }}>
        <div style={{ background: "linear-gradient(145deg,#111827,#1e293b)", borderRadius: 14, padding: "16px 14px", border: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ fontSize: 20, marginBottom: 8 }}>✨</div>
          <div style={{ fontFamily: "'Sora',sans-serif", fontSize: 13.5, fontWeight: 700, color: "#C9A84C", marginBottom: 4 }}>Upgrade to Pro</div>
          <p style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", lineHeight: 1.6, marginBottom: 10 }}>Unlock analytics, exports & premium features.</p>
          <button style={{ width: "100%", height: 34, borderRadius: 9, background: "linear-gradient(135deg,#C9A84C,#e6c96a)", color: "#111", border: "none", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
            Upgrade Now →
          </button>
        </div>
      </div>
      ...
      <div style={{ padding: "0 10px 14px", flexShrink: 0 }}>
        <button onClick={logout} className="btn btn-danger" style={{ width: "100%", justifyContent: "center", height: 40, borderRadius: 10, fontSize: 13.5 }}>
          <LogoutIcon /> Log out
        </button>
      </div>
    </aside>
